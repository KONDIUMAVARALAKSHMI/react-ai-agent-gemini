...
The available tools are described to Gemini using function declarations. Each schema contains:

Tool Name | Description | Parameters
---|---|---
`get_weather` | Retrieves weather conditions for a city. | `city` (string, required)
`calculate` | Evaluates a mathematical expression. | `expression` (string, required)
`write_file` | Writes text content to a file. | `filename` (string, required), `content` (string, required)
`read_file` | Reads a previously written file. | `filename` (string, required)

This allows Gemini to determine how the tools should be called and structured.
...
- `STEP` header
- `[THOUGHT]` (optional model-generated reasoning summary or agent trace)
- `[ACTION]` (when calling a tool, showing arguments)
- `[OBSERVATION]` (showing tool result)

> [!NOTE]
> The `[THOUGHT]` section displays the model's text response summarizing its next step or planning reasoning, rather than any hidden or private chain-of-thought.
